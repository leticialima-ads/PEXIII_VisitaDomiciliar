...
class DateUtilsHelper {
  ...
  static final DateFormat _dateFormatISO =
  ...
  static final DateFormat _monthYearFormat =
      DateFormat('MM/yyyy');

  static final DateFormat _yearFormat =
      DateFormat('yyyy');
  ...
  /// Parse de string para DateTime com hora
  static DateTime? parseDateTime(
    String dateStr,
  ) {
    try {
      return _dateTimeFormat.parse(
        dateStr,
      );
    } catch (e) {
      return null;
  ...
  static bool isMulherIdadeFertil(
    DateTime dataNascimento,
  ) {
  ...
  /// Homem em idade para vasectomia
  static bool isHomemIdadeVasectomia(
    DateTime dataNascimento,
  ) {
    final idade = calcularIdade(
      dataNascimento,
  ...
  static bool isConsultaDiabetesAtrasada(
  ...
  static bool isConsultaHipertensaoAtrasada(
    DateTime? dataConsulta,
  ) {
    return isAtrasado(
      dataConsulta,
  ...
  static bool isPreventivoAtrasado(
    DateTime? dataPreventivo,
  ) {
    return isAtrasado(
      dataPreventivo,
      365,
  ...
  static int mesesDesde(
    DateTime data,
  ) {
    final hoje = DateTime.now();

    int meses =
        (hoje.year - data.year) * 12 +
  ...
  static DateTime getInicioDoDia(
    DateTime date,
  ) {
    return DateTime(
      date.year,
  ...
  static List<DateTime> getDatasPeriodo(
    DateTime inicio,
    DateTime fim,
  ) {
    List<DateTime> datas = [];
  ...
  static bool isDataNascimentoValida(
    DateTime data,
  ) {
    return data.isBefore(
          DateTime.now(),
        ) ||
        data.isAtSameMomentAs(
          DateTime.now(),
        );
  }

  /// Validar data de consulta
  static bool isDataConsultaValida(
    DateTime data,
  ) {
    return data.isBefore(
          DateTime.now(),
  ...
  /// Validar data de visita
  static bool isDataVisitaValida(
    DateTime data,
  ) {
    return data.isBefore(
          DateTime.now(),
